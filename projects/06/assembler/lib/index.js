...
var SymbolTable = function SymbolTable() {
  ...
  _defineProperty(this, "nextAvailableAddress", 16);

  _defineProperty(this, "addLabel", function (symbol, address) {
    _this.table[symbol] = address;
  });

  _defineProperty(this, "addVariable", function (symbol) {
    _this.table[symbol] = _this.nextAvailableAddress;
    _this.nextAvailableAddress++;
  });
  ...
  _defineProperty(this, "getAddress", function (symbol) {
    if (!_this.contains(symbol)) {
      throw new Error("Symbol not found: ".concat(symbol));
    }
...
var Parser = function Parser(input, output) {
  var _this = this;
  ...
  _defineProperty(this, "symbolTable", void 0);

  _defineProperty(this, "fileContent", []);

  _defineProperty(this, "currentLine", 0);

  _defineProperty(this, "getCommandType", function (command) {
    if (command.includes('@')) {
  ...
  _defineProperty(this, "getCommandGroup", function (command) {
    ...
    if (command.includes('=')) {
      ...
      dest = _command$split2[0];
      comp = _command$split2[1];
    }

    if (comp.includes(';')) {
      var _comp$split = comp.split(';');

      var _comp$split2 = _slicedToArray(_comp$split, 2);
  ...
  _defineProperty(this, "generateCodeFromCommand", function (command) {
    ...
    switch (commandType) {
      case CommandType.address:
        var cleaned = command.slice(1);

        var _int = parseInt(cleaned);

        var address = _int >= 0 ? _int : _this.symbolTable.getAddress(cleaned);
      ...
      default:
        throw new Error("Unable to generate code from ".concat(command));
    }
  });

  _defineProperty(this, "parse", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            return _context.abrupt("return", new Promise(function (resolve, reject) {
              _this.reader.on('line', function (line) {
                var trimmed = line.trim(); // ignore comments and whitespace
                ...
                var command = trimmed.split('//')[0].replace(/\s/g, '');

                var type = _this.getCommandType(command);

                if (type === CommandType.label) {
                  // assign label to line after
                  var labelName = command.replace(/[\(\)]/g, '');

                  _this.symbolTable.addLabel(labelName, _this.fileContent.length);
                } else {
                  // potentially inefficient, might starve with huge files
                  // could write to a file instead if it becomes a problem
                  _this.fileContent.push(command);
                }
              }).on('close', function () {
                resolve();
              }).on('error', function () {
  ...
  _defineProperty(this, "write", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
    var parse, fileContent, generateCodeFromCommand, outputStream, getCommandType, currentLine, hasMoreCommands, command, type, addr, convertedCommand;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            parse = _this.parse, fileContent = _this.fileContent, generateCodeFromCommand = _this.generateCodeFromCommand, outputStream = _this.outputStream, getCommandType = _this.getCommandType;
            _context2.next = 3;
            return parse();
          ...
          case 3:
            ...
            while (hasMoreCommands) {
              command = fileContent[currentLine];
              type = getCommandType(command);
              addr = command.slice(1);

              if (type === CommandType.address && !(parseInt(addr) >= 0) && !_this.symbolTable.contains(addr)) {
                _this.symbolTable.addVariable(addr);
              }

              convertedCommand = generateCodeFromCommand(command);
              outputStream.write(( '') + convertedCommand + '\n');
              currentLine++;
...
var assemble = function assemble() {
  ...
  var outputStream = fs.createWriteStream("./dist/".concat(path.basename(assemblyFile, path.extname(assemblyFile)), ".hack"));
